...
fun getSSLContextFromPFXFile(path: String, certPath: String, pass: String): SSLContext {
    ...
    keystore.load(PiggProxy.javaClass.classLoader.getResourceAsStream(path), pass.toCharArray())

    val factory = KeyManagerFactory.getInstance(KeyManagerFactory.getDefaultAlgorithm())
    factory.init(keystore, pass.toCharArray())

    val trustStore = KeyStore.getInstance("JKS")
    trustStore.load(PiggProxy.javaClass.classLoader.getResourceAsStream(certPath), null)
